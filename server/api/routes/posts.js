...
router.get('/npages', async (req, res) => {
    try {
        const nPosts = await pool.query('SELECT COUNT(*) FROM posts')
        const nPages = nPosts.rows[0].count/ 8
        const nRedondeado = Math.ceil(nPages)

        return res.json({ success: true, message: 'Number of pages:', nRedondeado }).status(200)

    } catch (error) {
        return res.json({ success: false, message: 'nPages dosent load' + JSON.stringify(error) }).status(200)
    }
})
...
router.post('/', verifyToken, async (req, res) => {
    try {
        ...
        if (existeAlbum.rowCount > 0) {
            ...
            if (req.body.category && req.body.link && req.body.album && req.body.band && req.body.description && req.user.name && req.body.album_date) {
                ...
                return res.json({ success: false, message: 'Every field is required!' })
            }
        }
...
//numero de pagignas q significan todos los posts
router.get('/MyPosts/npages', verifyToken, async (req, res) => {
    try {
        const id = req.user.id

        const nPosts = await pool.query('SELECT COUNT(*) FROM posts WHERE user_id = $1', [id])
        const nPages = nPosts.rows[0].count/ 8
        const nRedondeado = Math.ceil(nPages)
...
router.get('/MyPosts', verifyToken, async (req, res) => {
    try {
        ...
        const lastIndex = firstIndex + 8
        const id = req.user.id
        const posts = await pool.query('SELECT * FROM posts WHERE user_id = $1 ORDER BY date DESC', [id])
        const array = posts.rows
        const postsPage = array.slice(firstIndex, lastIndex)

       return res.json({ succes: true, message: 'Posts done by' + req.user.name, postsPage }).status(200)
    } catch (error) {
        return res.json({ success: false, message: 'Database Problem' + JSON.stringify(error) }).status(400)
    }
})
